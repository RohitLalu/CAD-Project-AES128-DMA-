...
} -width {1.6} -pitch {40.0} -offset {13.570}
add_pdn_stripe -grid {grid} -layer {met5
} -width {1.6} -pitch {40.0} -offset {13.600}

# Power ring
add_pdn_ring -grid {grid} -layers {met4
 met5
} -widths {5.0 5.0} -spacings {2.0 2.0} -core_offsets {5.0 5.0}

# Connections
add_pdn_connect -grid {grid} -layers {met1 met4
}
add_pdn_connect -grid {grid} -layers {met4
 met5
}
